    ...
    private static void XrayerWarn(String xrayername) //Sends a warning message to an xrayer by name
    {
        for (Player player : Bukkit.getOnlinePlayers()) {
            if (player.hasPermission("AXH.XrayerWarning"))
                player.sendMessage(ChatColor.translateAlternateColorCodes('&', LocaleManager.get().getString("MessagesPrefix") + " " + PlaceholderManager.SubstitutePlayerNameAndColorCodePlaceholders(LocaleManager.get().getString("AutoHandledPlayer"), xrayername)));
        }
    }
                ...
                Bukkit.getScheduler().runTaskAsynchronously(mainClass, () -> mainClass.mm.StorePlayerData(player, new StorePlayerDataCallback() {
                    ...
                    public void onInsertDone(int handleTimes) {
                        ...
                        if (section != null && section.contains(String.valueOf(handleTimes))) {
                            ConfigurationSection subSection = mainClass.getConfig().getConfigurationSection("CommandsExecutedOnXrayerDetected." + handleTimes); //Gets the specific commands to execute if coinciding with amount of times detected
                            if (subSection != null) {
            ...
            if (belongings != null) //Belongings could be null when extracted from database, since StoreCopy option exists and can be false
            {
                ...
                for (int i = 0; i < 36; i++) {
                    ...
                    if (belongings[i] != null && belongings[i].getType() != Material.AIR && target.getInventory().firstEmpty() != -1)
                        ...
                        DropItemAtPlayerLocation(belongings[i], target); //No space, drop on floor.
                }
                //Return equipment
                if (target.getEquipment().getItemInOffHand().getType().equals(Material.AIR))
                    target.getEquipment().setItemInOffHand(belongings[36]); //Check if nothing in slot
                else DropItemAtPlayerLocation(belongings[36], target); //No space, drop on floor.
                if (target.getEquipment().getBoots() == null)
                    target.getEquipment().setBoots(belongings[40]); //Same with rest...
