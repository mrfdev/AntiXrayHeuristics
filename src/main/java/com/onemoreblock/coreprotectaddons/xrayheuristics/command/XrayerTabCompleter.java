...
public class XrayerTabCompleter implements TabCompleter {
    private final XRayHeuristicsModule plugin;

    public XrayerTabCompleter(@NonNull XRayHeuristicsModule plugin) {
        this.plugin = plugin;
    }
    ...
    public List<String> onTabComplete(
        ...
        if (args.length == 3 && args[0].equalsIgnoreCase("debug") && args[1].equalsIgnoreCase("set")) {
            return filterSuggestions(DebugSubcommand.getConfigKeys(), args[2]);
        }

        if (args.length == 4 && args[0].equalsIgnoreCase("debug") && args[1].equalsIgnoreCase("set")) {
            return filterSuggestions(DebugSubcommand.getValueSuggestions(args[2].toLowerCase(Locale.ROOT)), args[3]);
        }
