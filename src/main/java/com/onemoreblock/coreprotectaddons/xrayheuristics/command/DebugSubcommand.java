...
public final class DebugSubcommand {
    ...
    public static boolean handle(
            ...
            String @NonNull [] args,
            @NonNull XRayHeuristicsModule plugin
    ) {
        if (!HelpSubcommand.hasAdminAccess(sender)) {
    ...
    private static void sendOverview(@NonNull CommandSender sender, @NonNull XRayHeuristicsModule plugin) {
        ...
        sendLine(sender, "&7Feature: &f" + plugin.getFeatureName() + " (" + plugin.getFeatureId() + ")");
        sendLine(sender, "&7Host plugin: &f" + plugin.getHostPlugin().getPluginMeta().getName()
                + " " + plugin.getHostPlugin().getPluginMeta().getVersion());
        ...
        sendLine(sender, "&7Description: &f" + plugin.getFeatureDescription());
    ...
    private static void sendConfig(@NonNull CommandSender sender, @NonNull XRayHeuristicsModule plugin) {
        sendLine(sender, "&8&m------------------------------------------------");
    ...
    private static @NonNull String formatConfigValue(@NonNull XRayHeuristicsModule plugin, @NonNull DebugConfigKey configKey) {
