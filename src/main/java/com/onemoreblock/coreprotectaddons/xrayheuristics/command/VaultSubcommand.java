...
public class VaultSubcommand {
    private static final LegacyComponentSerializer LEGACY_SERIALIZER = LegacyComponentSerializer.legacyAmpersand();
    private static final String ADMIN_PERMISSION = "xrayheuristics.admin";

    static void execute(CommandSender sender, XRayHeuristicsModule module) {
        if (sender instanceof Player player) {
            if (player.hasPermission(ADMIN_PERMISSION) || player.hasPermission("AXH.Commands.Vault")) {
                module.handledPlayerVault.UpdateXrayerInfoLists(player, 0); // Update vault and open UI for player
            } else {
